...
import { useUsers, usePeriodos } from '@/hooks';
...
export const ReservaFilters = ({ 
  ...
  // Obtener periodos activos para el filtro de fecha de envío
  const { periodos } = usePeriodos({ isActive: true });
  ...
  return (
    <div className="space-y-4 w-full overflow-hidden">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 w-full">
        ...
        {/* Fecha de envío del periodo */}
        <Select
          label="Fecha de Envío"
          value={filters.periodoId?.toString() || ''}
          onChange={(e) => onFilterChange({ 
            periodoId: e.target.value ? parseInt(e.target.value, 10) : undefined 
          })}
          options={[
            { value: '', label: 'Todas las fechas de envío' },
            ...(periodos || []).map((periodo) => ({
              value: periodo.id.toString(),
              label: formatDate(periodo.fechaEnvio),
            })),
          ]}
        />
      </div>
